...
func TestMissingRelationIfNotExists(t *testing.T) {
	t.Parallel()
	...
	t.Run("Should find violation", func(t *testing.T) {
		...
		res, err := missingRelationIfNotExists(tree, testCode, testSlug, testHelp)
	...
	t.Run("Should find multiple violations", func(t *testing.T) {
		...
		b.WriteString("CREATE TABLE pgcheck_two (id integer PRIMARY KEY);")
		...
		assert.EqualValues(t, 0, res[0].StmtStart)
		assert.EqualValues(t, 99, res[1].StmtStart)
	})
...
func TestMissingIndexIfNotExists(t *testing.T) {
	t.Parallel()
	...
	t.Run("Should find violation", func(t *testing.T) {
		...
		res, err := missingIndexIfNotExists(tree, testCode, testSlug, testHelp)
		require.NoError(t, err)
		require.Len(t, res, 1)
	...
	t.Run("Should find multiple violations", func(t *testing.T) {
		...
		b.WriteString("ALTER TABLE pgcheck RENAME COLUMN value TO new_value;\n")
		b.WriteString("CREATE INDEX pgcheck_key_two ON pgcheck(id);")
		tree := mustParse(t, b.String())
		require.Len(t, tree.Stmts, 3)
	...
	t.Run("Should not return statements that have safe guard", func(t *testing.T) {
		t.Parallel()

		var b strings.Builder
		b.WriteString("CREATE INDEX IF NOT EXISTS pgcheck_key ON pgcheck(id);")
		tree := mustParse(t, b.String())
		require.Len(t, tree.Stmts, 1)
	...
	t.Run("Should not return statements when index is not named", func(t *testing.T) {
		t.Parallel()
